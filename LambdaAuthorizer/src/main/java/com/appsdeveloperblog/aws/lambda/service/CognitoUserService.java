...
public class CognitoUserService {
    ...
    public JsonObject getUserByUsername(String userName, String poolId) {
        ...
        if (!adminGetUserResponse.sdkHttpResponse().isSuccessful()) {
            throw new RuntimeException("Unsuccessful result. Status code: " + adminGetUserResponse.sdkHttpResponse().statusCode());
        }
        List<AttributeType> attributes = adminGetUserResponse.userAttributes();
        attributes.forEach(attribute -> userDetails.addProperty(attribute.name(), attribute.value()));
